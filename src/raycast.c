 ...
 * @param r The raycast struct
 * @dev: Calculating the Delta Distance
 ...
 * @param p The player struct
 * @param r The raycast struct
 * @dev: We also take note of the side of the wall we hit (0 for x, 1 for y).
...
/**
 * @brief Calculating the Wall Height. Calculate height of line to draw on 
 * screen.
 * @param p The player struct
 * @param r The raycast struct
 * @dev: Calculate lowest and highest pixel to fill in current stripe.
 ...
 * @dev: create pixel map, that reserve memory.
 * camera_x -> x-coordinate in camera space
 * ray_dirx / diry -> calculate ray position and direction
 * map_x / map_y -> which box of the map we're in
 * delta_dist_x / dist_y  -> length of ray from x or y-side to next x or y-side
